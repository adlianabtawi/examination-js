 /**
 * REPRESENTERAR EN PRODUKT (Model)
 * @class Product
 */
class Product {
  /** @param {Object} data */
...
/**
 * HANTERAR VARUKORGENS LOGIK (Logic)
 * @class ShoppingCart
 */
class ShoppingCart {
  constructor() {
  ...
  /** Totalt antal produkter (summa av qty) */
  get count() {
...
class App {
  ...
  /** Hjälpfunktion: hämta produkt med id */
  getProductById(id) {
    return this.products.find(p => p.id === id);
  }

  /** Hjälpfunktion: hämta qty i varukorgen för en produkt */
  getQtyInCart(productId) {
    const item = this.cart.items.find(i => i.id === productId);
    return item ? item.qty : 0;
  }

  /** Öppnar modalen och renderar Lucide-ikoner */
  openModal() {
  ...
  cartTemplate(itemsHtml) {
    return `
      <div id="cart-view">
        ...
        <div class="summary">
          <div class="summary-row"><span>Delsumma</span><span>${this.cart.total} kr</span></div>
          ...
          <div class="cart-item">
            ...
            <div class="cart-item-info">
              ...
              <div style="margin-top:15px; display:flex; gap:12px; align-items:center;">
                <button class="qty-btn-small qty-mod" data-id="${item.id}" data-change="-1">-</button>
            ...
            <div style="text-align:right;">
              ...
              <button class="btn-link-red qty-mod" data-id="${item.id}" data-change="-${item.qty}">Ta bort</button>
            </div>
          </div>
        `
      )
